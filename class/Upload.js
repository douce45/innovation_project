...
class Upload {
  constructor() {
    this.destinationPath =
      path.resolve("./") + path.sep + "public" + path.sep + "uploads";
  }
  async upload(
    file,
    withThumb = true,
    ...
    try {
      const extname = fileDestination
        ? path.extname(fileDestination)
        : path.extname(file.name);
      const defaultFileName = Date.now() + extname;
      const finalFileName = fileDestination
        ? path.basename(fileDestination)
        : defaultFileName;
      const thumbName =
        path.parse(finalFileName).name + "_thumb" + path.extname(finalFileName);
      const destinationFolder = fileDestination
        ? path.dirname(fileDestination)
        : this.destinationPath;
